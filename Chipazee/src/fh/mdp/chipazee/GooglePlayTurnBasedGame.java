...
public class GooglePlayTurnBasedGame implements TurnbasedGame, GameHelperListener {
	...
	{
		Intent intent = Games.TurnBasedMultiplayer.getInboxIntent(mHelper.getApiClient());
        ...
        switch (turnStatus) {
            ...
            case TurnBasedMatch.MATCH_TURN_STATUS_MY_TURN:
                ...
                if (mTurn.turnNumber < 2) {
                    Games.Achievements.unlock(mHelper.getApiClient(), mActivity.getResources().getString(R.string.achievement_started_a_game));
                }
	...
	public void takeTurn(Turn turn) {
		byte[] data = mTurn.serialize();
		if (data == null)
			showWarning("Serialization failed", "An error occoured and your turn can not be finished.");
		else
			Games.TurnBasedMultiplayer.takeTurn(mHelper.getApiClient(), mMatch.getMatchId(), data, getNextParticipant().getParticipantId());
	}
	...
	public void cancelMatch() {
		Games.TurnBasedMultiplayer.cancelMatch(mHelper.getApiClient(), mMatch.getMatchId());
	}

	@Override
	public void leaveMatch() {
		Games.TurnBasedMultiplayer.leaveMatchDuringTurn(mHelper.getApiClient(), mMatch.getMatchId(), getNextParticipant().getParticipantId());
    ...
    public Participant getCurrentParticipant() {
        return getParticipantForPlayerId(Games.Players.getCurrentPlayerId(mHelper.getApiClient()));
    }
    ...
    public Participant getNextParticipant()
    {
    	...
    	for (myIndex = 0; myIndex < mMatch.getParticipants().size(); myIndex++)
    	{
    		...
    		if (p.getPlayer() != null && p.getPlayer().getPlayerId() == Games.Players.getCurrentPlayerId(mHelper.getApiClient()))
    			break;
    	}
